...
### 4. Run inference over a folder and save JSON + overlays
...
### 5. Raw forward pass (custom decode pipeline)
...
`train_model()` saves to `output_dir/checkpoints/epoch_XXXX.pt`, containing `model_state_dict`, `optimizer_state_dict`, `epoch`, and `cfg`. Resuming is controlled entirely through the config:
